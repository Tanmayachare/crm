...
const currentRouteName = computed(() => {
  ...
  if (route.path.startsWith('/view/')) return `${route.params.tableName} Records`;
  return 'Workspace';
});
...
<template>
  <div class="h-screen bg-gray-50 flex font-sans overflow-hidden">
    ...
    <!-- Enterprise Sidebar -->
    <aside class="w-64 bg-[#1a202c] text-gray-300 flex flex-col flex-shrink-0 shadow-xl z-20">
      <div class="h-16 flex items-center px-6 border-b border-gray-700 bg-[#161b22]">
        <h2 class="text-xl font-bold tracking-wider text-white flex items-center gap-2">
          <svg class="w-6 h-6 text-blue-500" fill="currentColor" viewBox="0 0 24 24"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
          CRM Pro
        </h2>
      </div>
      ...
      <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1 custom-scrollbar">
        ...
        <div v-if="objects.length" class="px-3 mt-6 mb-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">Custom Objects</div>
        
        <RouterLink v-for="objs in objects" :key="objs" :to="'/view/' + objs" active-class="bg-blue-600 text-white font-medium shadow-md"
          class="flex items-center gap-3 py-2 px-3 rounded-md transition duration-150 ease-in-out hover:bg-gray-800 hover:text-white group">
      ...
      <div class="p-4 border-t border-gray-700 bg-[#161b22]">
        <button class="w-full flex items-center justify-center gap-2 py-2 text-sm text-gray-400 hover:text-white hover:bg-gray-800 rounded-md transition">
    ...
    <main class="flex-1 flex flex-col min-w-0 bg-gray-50">
      ...
      <header class="h-16 bg-white border-b border-gray-200 px-6 flex justify-between items-center flex-shrink-0 z-10">
        <div class="flex items-center gap-4">
          <h1 class="text-lg font-semibold text-gray-800">{{ currentRouteName }}</h1>
        </div>
        ...
        <div class="flex items-center gap-4">
          <div class="flex items-center gap-3 cursor-pointer hover:bg-gray-50 py-1 px-2 rounded-md transition">
            ...
            <div class="w-9 h-9 bg-gradient-to-br from-blue-500 to-blue-700 rounded-full flex items-center justify-center text-white font-bold shadow-sm border-2 border-white ring-2 ring-gray-100">
              T
            </div>
          </div>
        </div>
      ...
      <div class="bg-white border-b border-gray-200 px-6 flex space-x-1 flex-shrink-0">
         <RouterLink to="/" active-class="border-b-2 border-blue-500 text-blue-600" class="px-4 py-2.5 text-sm font-medium text-gray-600 hover:text-gray-900 border-b-2 border-transparent transition-colors whitespace-nowrap">
            Dashboard
         </RouterLink>
         <RouterLink to="/leads" active-class="border-b-2 border-blue-500 text-blue-600" class="px-4 py-2.5 text-sm font-medium text-gray-600 hover:text-gray-900 border-b-2 border-transparent transition-colors whitespace-nowrap">
            Leads
         </RouterLink>
...
<style>
/* Custom Scrollbar for a cleaner enterprise look */
